...
<div class="container">
	...
	<ul>
		...
		<li>첨부파일 : <a href="${url}/upload/${dataVO.filename1}" download>${dataVO.filename1 }</a>
			<c:if test="${dataVO.filename2!=null && dataVO.filename2!='' }">
				, <a href="${url}/upload/${dataVO.filename2}" download>${dataVO.filename2}</a>
			</c:if>
		</li>
	</ul>
	<c:if test="${dataVO.userid==logId}">
		<a href="${url }/data/dataEdit?no=${dataVO.no}">수정</a>
